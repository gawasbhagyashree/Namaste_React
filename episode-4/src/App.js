...
/**
 * 1) Writing style components in Logo
 * 
 * props => are arguments to a function
 * 2) Passing and Receiving props
 * 
 * 3) Config UI => fetching live data from Swiggy API
 *   - copying just one restaurant data into const resObj ={ }
 * 
 * 4) Using template literals, && and ternary operator ?:: in h3 image-txt
 *?    { data?. data?...header &&  data2 ?. data2 ?......subheader? `${data.data....header} ${data2.data2.....subheader}`: '' }
 * 
 * 5) writing img src where half string is constant and half is dynamic coming from API
 *?    => <img src= {`constant${dynamic}`} />
 * 
 * 6) Destructuring the props that are received
 * 
 * 
 * 7) using map instead of:
 * <RestaurantCard
 *  resData={resObj.restaurants[1]
 *  />
 *  
 * ?     =>  consider the array i.e restaurants[1] as x
 * ?     then, resObj. restaurants.map((x)=>{
 * ?      return(
 * ?      <RestaurantCard 
 * ?       resData = {x}
 * ?      )
 *?      })
 *
 * 8) using keys in mapped array items:
 * Avoid using index as key as much as possible
 * 
 * either this: 
 * 
 ...
 *
 * OR
 * {resObj.restaurants.map((x, index) => {
        return (
          <RestaurantCard
